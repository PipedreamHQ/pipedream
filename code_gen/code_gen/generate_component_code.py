...
def main(app, prompt, verbose=False):
    ...
    if row is None:
        return langchain_helpers.no_docs(app, prompt, verbose)
    ...
    if row['docs_url']:
        ...
        if len(rows or []) == 0:
            ...
            if verbose:
                print("using api reference docs")
        ...
        if len(rows or []) == 0:
            ...
            if verbose:
                print("using openapi docs")
    else:
        use_docs = False

    if use_docs:
        result = langchain_helpers.ask_agent(prompt, docs, verbose)
        if result == "I don't know":
            if verbose:
                print("trying again without docs")
            result = langchain_helpers.no_docs(app, prompt, verbose)
    else:
        result = langchain_helpers.no_docs(app, prompt, verbose)
