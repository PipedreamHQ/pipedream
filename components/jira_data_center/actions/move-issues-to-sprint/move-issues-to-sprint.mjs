import jiraDataCenter from "../../jira_data_center.app.mjs";

export default {
  name: "Move Issues To Sprint",
  description: "Moves issues to a sprint. [See the documentation](https://developer.atlassian.com/server/jira/platform/rest/v10002/api-group-sprint/#api-agile-1-0-sprint-sprintid-issue-post)",
  key: "jira_data_center-move-issues-to-sprint",
  version: "0.0.1",
  type: "action",
  annotations: {
    destructiveHint: false,
    openWorldHint: true,
    readOnlyHint: false,
  },
  props: {
    jiraDataCenter,
    boardId: {
      propDefinition: [
        jiraDataCenter,
        "boardId",
      ],
    },
    sprintId: {
      propDefinition: [
        jiraDataCenter,
        "sprintId",
        (c) => ({
          boardId: c.boardId,
          excludeClosed: true,
        }),
      ],
    },
    issueIds: {
      propDefinition: [
        jiraDataCenter,
        "issueId",
        (c) => ({
          boardId: c.boardId,
        }),
      ],
      type: "string[]",
      label: "Issue IDs",
      description: "The IDs of the issues to move",
    },
  },
  async run({ $ }) {
    const response = await this.jiraDataCenter.moveIssuesToSprint({
      $,
      sprintId: this.sprintId,
      data: {
        issues: this.issueIds,
      },
    });
    $.export("$summary", `Successfully moved ${this.issueIds.length} issues to sprint with ID \`${this.sprintId}\``);
    return response;
  },
};
