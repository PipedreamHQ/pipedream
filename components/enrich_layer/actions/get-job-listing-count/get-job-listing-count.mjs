...
export default {
  ...
  async run({ $ }) {
    const response = await this.enrichlayer.getJobListingCount({
      $,
      params: {
    ...
    $.export("$summary", `Successfully retrieved job listing count${this.searchId
      ? ` for company ${this.searchId}`
      : ""}`);
    return response;
  },
};
