...
export default {
  ...
  props: {
    ...
    channelTypes: {
      type: "string[]",
      label: "Channel Types",
      description: "The types of channels to list.",
      options: constants.CHANNEL_TYPES_OPTIONS,
      optional: true,
    },
    pageSize: {
      propDefinition: [
  ...
  async run({ $ }) {
    ...
    const types = this.channelTypes
      ? Array.isArray(this.channelTypes)
        ? this.channelTypes.join(",")
        : this.channelTypes
      : constants.CHANNEL_TYPES_OPTIONS.map((option) => option.value).join(",");
