...
import {
  apFirst as apFirst_, apS as apS_, apSecond as apSecond_,
} from "./Apply";
import {
  bind as bind_, chainFirst as chainFirst_,
} from "./Chain";
...
import {
  bindTo as bindTo_, flap as flap_, let as let__,
} from "./Functor";
...
import {
  filterE as filterE_, wiltDefault, witherDefault,
} from "./Witherable";
 ...
 * @since 2.5.0
 */
export var makeBy = function (n, f) { return (n <= 0
  ? empty
  : RNEA.makeBy(f)(n)); };
/**
 ...
 * @since 2.11.0
 */
export var fromOption = function (ma) { return (_.isNone(ma)
  ? empty
  : [
    ma.value,
  ]); };
 ...
 * @since 2.11.0
 */
export var fromEither = function (e) { return (_.isLeft(e)
  ? empty
  : [
    e.right,
  ]); };
 ...
 * @since 2.5.0
 */
export var head = function (as) { return (isNonEmpty(as)
  ? _.some(RNEA.head(as))
  : _.none); };
/**
 ...
 * @since 2.5.0
 */
export var last = function (as) { return (isNonEmpty(as)
  ? _.some(RNEA.last(as))
  : _.none); };
/**
...
export var modifyAt = function (i, f) {
  return function (as) {
    return isOutOfBound(i, as)
      ? _.none
      : _.some(unsafeUpdateAt(i, f(as[i]), as));
  };
};
/**
 ...
 * @since 2.5.0
 */
export var reverse = function (as) { return (as.length <= 1
  ? as
  : as.slice().reverse()); };
/**
...
export function zip(as, bs) {
  ...
  return zipWith(as, bs, function (a, b) { return [
    a,
    b,
  ]; });
}
...
export var prependAll = function (middle) {
  ...
  return function (as) { return (isNonEmpty(as)
    ? f(as)
    : as); };
};
/**
...
export var getSemigroup = function () { return ({
  concat: function (first, second) { return (isEmpty(first)
    ? second
    : isEmpty(second)
      ? first
      : first.concat(second)); },
 ...
 * @since 2.5.0
 */
export var fromArray = function (as) { return (isEmpty(as)
  ? empty
  : as.slice()); };
// -------------------------------------------------------------------------------------
// utils
