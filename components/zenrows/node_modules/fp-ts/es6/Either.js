...
import {
  chainOptionK as chainOptionK_, filterOrElse as filterOrElse_, fromOption as fromOption_, fromOptionK as fromOptionK_, fromPredicate as fromPredicate_,
} from "./FromEither";
import {
  dual, flow, identity, pipe,
} from "./function";
import {
 ...
 * @since 2.14.0
 */
export var flatMap = /*#__PURE__*/ dual(2, function (ma, f) { return (isLeft(ma)
  ? ma
  : f(ma.right)); });
...
export var getShow = function (SE, SA) { return ({
  show: function (ma) { return (isLeft(ma)
...
export var getSemigroup = function (S) { return ({
  concat: function (x, y) { return (isLeft(y)
    ? x
    : isLeft(x)
      ? y
...
export var getCompactable = function (M) {
  ...
  return {
    URI: URI,
    _E: undefined,
    compact: function (ma) { return (isLeft(ma)
      ? ma
      : ma.right._tag === "None"
        ? empty
...
export var getFilterable = function (M) {
  ...
  return {
    ...
    partitionMap: function (ma, f) {
      ...
      var e = f(ma.right);
      return isLeft(e)
        ? separated(right(e.left), empty)
        : separated(empty, right(e.right));
    },
  };
};
/**
...
export var getAltValidation = function (SE) { return ({
  ...
  alt: function (me, that) {
    ...
    var ea = that();
    return isLeft(ea)
      ? left(SE.concat(me.left, ea.left))
      : ea;
  },
}); };
/**
 ...
 * @since 2.0.0
 */
export var swap = function (ma) { return (isLeft(ma)
  ? right(ma.left)
  : left(ma.right)); };
/**
