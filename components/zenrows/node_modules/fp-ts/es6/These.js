...
import {
  fromOption as fromOption_, fromOptionK as fromOptionK_, fromPredicate as fromPredicate_,
} from "./FromEither";
import {
  identity, pipe,
} from "./function";
...
export function leftOrBoth(e) {
  return function (ma) { return (_.isNone(ma)
    ? left(e)
    : both(e, ma.value)); };
}
// TODO: make lazy in v3
...
export function rightOrBoth(a) {
  return function (me) { return (_.isNone(me)
    ? right(a)
    : both(me.value, a)); };
}
/**
    ...
    return function (ta) {
      return isLeft(ta)
        ...
        : isRight(ta)
          ...
          : F.map(f(ta.right), function (b) { return both(ta.left, b); });
    };
  };
};
/**
  ...
  return function (ta) {
    return isLeft(ta)
      ...
      : isRight(ta)
        ...
        : F.map(ta.right, function (b) { return both(ta.left, b); });
  };
};
/**
 * @category constructors
    ...
    return function (as) {
      ...
      for (var i = 1; i < as.length; i++) {
        ...
        if (isBoth(t_1)) {
          e = _.isNone(e)
            ? _.some(t_1.left)
            : _.some(S.concat(e.value, t_1.left));
        }
        out.push(t_1.right);
      }
      return _.isNone(e)
