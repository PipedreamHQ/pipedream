...
export default {
  key: "figranium-describe-schedule",
  name: "Describe Schedule",
  description: "Validate and preview a schedule config without saving it. [See the documentation](https://figranium.com/docs/api-authentication-and-secure-access).",
  version: "0.0.1",
  annotations: {
  ...
  async run({ $ }) {
    ...
    const response = await app.describeSchedule({
      $,
      taskId,
      data,
    });

    $.export("$summary", `Successfully described schedule for task \`${taskId}\`.`);
    return response;
  },
};
