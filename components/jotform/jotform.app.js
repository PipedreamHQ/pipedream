...
module.exports = {
  ...
  methods: {
    ...
    async getWebhooks(opts = {}) {
      ...
      return (
        await this._makeRequest({
          url: `https://api.jotform.com/form/${encodeURIComponent(
            formId,
          )}/webhooks`,
          method: `GET`,
          headers: {
    ...
    async deleteHook(opts = {}) {
      ...
      if (webhookIdx === -1) {
        console.log(
          `Did not detect ${endpoint} as a webhook registered for form ID ${formId}.`,
        );
        return;
      }
      ...
      return await this._makeRequest({
        url: `https://api.jotform.com/form/${encodeURIComponent(
          formId,
        )}/webhooks/${encodeURIComponent(webhookIdx)}`,
        method: `DELETE`,
        headers: {
          APIKEY: this.$auth.api_key,
        },
