// legacy_hash_id: a_NqinoP
import axios from "axios";

export default {
  key: "raindrop-retrieve-all-bookmarks",
  name: "Retrieve All Bookmarks From Raindrop Collection",
  description: "Retrieves all bookmarks from the specified Raindrop Collection, exhaustively paging through the results.",
  version: "0.1.1",
  type: "action",
  props: {
    raindrop: {
  ...
  async run() {
    // Raindrop API parameters
    // The page parameter is set to 0 and is incremented automatically in order to retrieve every page of results
    const {
      collectionId,
      sort,
      search,
    ...
    do {
      ...
      responseSize = response.data.items.length;

      if (responseSize) {
        bookmarks.push(...response.data.items);
        page++;
      }
