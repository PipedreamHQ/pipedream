...
export default {
  ...
  props: {
    ...
    sheetId: {
      ...
      description: "The sheet the rows are copied from. Accepts a numeric sheet ID (e.g. `1234567890123456`), or a Smartsheet sheet URL, which is resolved to the ID for you. Use **List Sheets** to enumerate sheets, or **Search** to find one by name.",
    },
    rowIds: {
      type: "string",
      label: "Row IDs",
    ...
    destinationSheetId: {
      ...
      description: "The sheet the rows are copied into. Accepts a numeric sheet ID (e.g. `1234567890123456`), or a Smartsheet sheet URL, which is resolved to the ID for you. Use **List Sheets** to enumerate sheets, or **Search** to find one by name.",
    },
    include: {
      type: "string[]",
      label: "Include",
  ...
  async run({ $ }) {
    ...
    const response = await this.smartsheet.copyRows(sheetId, {
      $,
      params: {
    ...
    // Report what was copied: Ignore Rows Not Found skips missing IDs.
    const copied = response.rowMappings?.length ?? rowIds.length;
    $.export("$summary", `Copied ${copied} of ${rowIds.length} row(s) from sheet ${sheetId} to sheet ${destinationSheetId}`);
    return response;
  },
};
