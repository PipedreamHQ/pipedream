...
export default {
  ...
  props: {
    ...
    sheetId: {
      ...
      description: "The ID of the sheet to update (e.g. `1234567890123456`). Use **List Sheets** to find sheet IDs. A Smartsheet sheet URL is also accepted and resolved to the ID for you.",
    },
    name: {
      type: "string",
      label: "New Name",
  ...
  async run({ $ }) {
    ...
    const response = await this.smartsheet.updateSheetProperties(sheetId, {
      $,
      data,
    });
    $.export("$summary", `Updated sheet "${response.result?.name || sheetId}"`);
    return response;
  },
};
