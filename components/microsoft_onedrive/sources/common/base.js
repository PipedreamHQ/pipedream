...
module.exports = {
  ...
  hooks: {
    async deploy() {
      const params = {
        $top: MAX_INITIAL_EVENT_COUNT,
      };
      const deltaLink = await this.microsoft_onedrive.getDeltaLink(params);
  ...
  methods: {
    ...
    async _createNewSubscription() {
      ...
      const hookId = await this.microsoft_onedrive.createHook(hookOpts);
