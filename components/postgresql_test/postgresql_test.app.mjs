...
export default {
  ...
  propDefinitions: {
    ...
    table: {
      ...
      async options({
        ...
        return this.getTables(
          this.getNormalizedSchema(schema),
          rejectUnauthorized,
        );
      },
    },
    column: {
      ...
      async options({
        table, schema, rejectUnauthorized,
      }) {
    ...
    value: {
      ...
      async options({
        ...
        return {
          options: await this.getColumnValues(
            table,
            column,
            limit,
            offset,
  ...
  methods: {
    ...
    async deleteRows(schema, table, column, value, rejectUnauthorized) {
      return this.executeQuery(
        {
          text: format(
            "DELETE FROM %I.%I WHERE %I = $1 RETURNING *",
            schema,
            table,
            column,
          ),
    ...
    getColumnsPlaceholders({
      ...
      return Object.keys(rowValues).map(
        (key, index) => `${key} = $${index + fromIndex}`,
      );
    },
    /**
