...
export default {
  ...
  methods: {
    ...
    async initialRows(schema, table, column, limit, rejectUnauthorized) {
      const rows = await this.postgresql.getInitialRows(
        schema,
        table,
        column,
        limit,
    ...
    async isColumnUnique(schema, table, column) {
      const query = format(
        "select count(*) <> count(distinct %I) as duplicate_flag from %I.%I",
        column,
        schema,
        table,
      );
      const hasDuplicates = await this.postgresql.executeQuery(
        query,
        this.rejectUnauthorized,
      );
