...
export default {
  ...
  name: "List Tabs",
  description: "List the tabs in a Google Doc — each tab's ID, title, position and nesting. A Google Doc can hold many tabs, and every editing tool writes to the **first** tab unless it is given a **Tab ID**, so call this first whenever the job names a tab (\"add this to the Budget tab\") or the document might have more than one. Returns `{documentId, tabCount, tabs: [{tabId, title, index, nestingLevel, parentTabId}]}`, in document order, with each parent followed by its nested child tabs. Pass a `tabId` from this list to **Get Document** to read one tab, or to **Insert Text**, **Write Table**, **Insert Image**, **Insert Table**, **Insert Page Break**, **Delete Table** or **Replace Text** to edit one. Use **Find Document** to resolve a document's name to its ID. This tool set can list tabs, add a tab (**Create Tab**) and read or write a named tab; it cannot rename, reorder or delete an existing tab — say so plainly rather than attempting a workaround. [See the documentation](https://developers.google.com/workspace/docs/api/how-tos/tabs)",
  version: "0.0.1",
  annotations: {
