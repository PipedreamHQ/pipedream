...
const testAction = {
  mockery: { drift, ...mockeryData }, // TEST
  ...
  async run({ $ }) {
    ...
    const {drift, email, name, phone} = this.mockery;
    ...
    const existingContact = await drift.methods.getContactByEmail({$, params : {email}});
