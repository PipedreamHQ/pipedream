...
**Jobs, clients, and operations currently implemented:** list/get/create/update/delete for jobs, companies, company contacts, job materials, job activities, staff, queues, categories, badges, notes, attachments (`dboattachment`), job payments, job contacts, and feedback.

Use **Make API Request** for endpoints not yet wrapped as first-class actions (for example materials, tasks, assets, locations, bundles, and allocation windows).
