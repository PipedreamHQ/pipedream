...
Connect your Alive5 account with an API key from the Alive5 dashboard settings. The key is sent using the `X-A5-APIKEY` header to the Alive5 API, and transiently to the Alive5 relay at `https://wmbb2krgd4yqmghwsxd3yamzey0ocqsz.lambda-url.us-east-1.on.aws` for subscription management. See the [Alive5 API documentation](https://www.alive5.com/api).
...
Alive5 does not sign deliveries, so the relay cannot prove that an incoming event came from Alive5. The Alive5-operated AWS relay durably queues accepted events in SQS. Failed deliveries are retried, preserving the event ID, and moved to a dead-letter queue after five receives. Pipedream deduplicates emitted events by that ID. Upstream Alive5 delivery remains single-attempt, and there is no exactly-once guarantee. Deactivating and reactivating the source creates a new upstream capability and delivery token. To change Pipedream's receiving URL, recreate the source.
