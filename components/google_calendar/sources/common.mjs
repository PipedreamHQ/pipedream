...
export default {
  ...
  methods: {
    ...
    async processEvents(event) {
      const intervalData = this.getIntervalData(event); console.log(intervalData);
      const config = this.getConfig(intervalData); console.log(config);
      const resp = await this.googleCalendar.getEvents(config); console.log(resp);

      const events = resp?.data?.items; console.log(events);
