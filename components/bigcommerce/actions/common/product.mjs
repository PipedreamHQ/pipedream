...
export default {
  props: {
    ...
    tax_class_id: {
      ...
      optional: true,
      async options({ page }) {
        const taxes = await this.app.getAllTaxClasses({
          page: page + 1,
        });
    ...
    brand_id: {
      ...
      optional: true,
      async options({ page }) {
        const { data } = await this.app.getAllBrands({
          page: page + 1,
        });
    ...
    related_products: {
      ...
      optional: true,
      async options({ page }) {
        const { data } = await this.app.getAllProducts({
          page: page + 1,
        });
        return data.map((item) => ({
          label: item.name,
    ...
    images: {
      type: "string[]",
      label: "Images",
      description: "Array of image strings. They can be either local paths or URLs. Limit of 8MB per file. If it is a URL it has a 255 character limit. Supported image file types are BMP, GIF, JPEG, PNG, WBMP, XBM, and WEBP. If it is a local path it should be previously downloaded to Pipedream E.g. (`/tmp/my-image.png`). [Download a file to the `/tmp` directory](https://pipedream.com/docs/code/nodejs/http-requests/#download-a-file-to-the-tmp-directory)",
      optional: true,
    },
  },
  methods: {
    getRequestFn() {
      throw new ConfigurationError("getRequestFn not implemented");
    },
    getRequestFnArgs() {
      throw new ConfigurationError("getRequestFnArgs not implemented");
    },
    getSummary() {
  ...
  async run({ $ }) {
    ...
    const imagesArray = utils.parseArray(images);
    const imageFiles = imagesArray?.filter((image) => image?.startsWith("/"));
    const imageUrls = imagesArray?.filter((image) => image?.startsWith("http"));
    ...
    const productResponse = await requestFn(args);

    if (!imageFiles.length) {
      $.export("$summary", getSummary(productResponse));
      return productResponse;
    }

    const productImages = imageFiles.map((imageFile) => app.createProductImage({
      $,
    ...
    const response = await app.getProduct({
      $,
      productId: productResponse.data.id,
      params: {
        include: "images",
