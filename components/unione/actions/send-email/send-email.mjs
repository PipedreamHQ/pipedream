...
export default {
  ...
  props: {
    ...
    trackLinks: {
      type: "string",
      label: "Track Links",
      description: "1 = click tracking is on (default), 0 = click tracking is off. To use track_links = 0, you need to ask UniOne support to enable this feature.",
    ...
    bypassGlobal: {
      type: "string",
      label: "Bypass Global",
      description: "If set to 1, the global unavailability list will be ignored. Even if the address was found to be unreachable while sending other UniOne users' emails, or its owner has issued complaints, the message will still be sent. The setting may be ignored for certain addresses.",
      options: TRACK_OPTIONS,
      optional: true,
    },
    bypassUnavailable: {
      ...
      description: "If set to 1, the current list of unsubscribed addresses for this account or project will be ignored. Works only if `Bypass Global` is set to 1. The setting is available only for users that have been granted the right to omit the unsubscribe link (to request, please contact [support](https://cp.unione.io/en/support?_gl=1*1msqb8a*_ga*MTgyNTM0MDM4OS4xNzYyODkzNzky*_ga_37TV6WM09S*czE3NjI4OTM3OTIkbzEkZzEkdDE3NjI4OTQ1NTAkajQ5JGwwJGg4ODQyOTkzMzQ.)).",
      options: TRACK_OPTIONS,
      optional: true,
    },
    bypassUnsubscribed: {
      ...
      description: "If set to 1, the current list of unsubscribed addresses for this account or project will be ignored. Works only if bypass_global is set to 1. The setting is available only for users that have been granted the right to omit the unsubscribe link.",
      options: TRACK_OPTIONS,
      optional: true,
    },
    bypassComplained: {
      ...
      description: "If set to 1, the user's or project's complaint list will be ignored. Works only if bypass_global is set to 1. The setting is available only for users that have been granted the right to omit the unsubscribe link.",
      options: TRACK_OPTIONS,
      optional: true,
    },
    idempotenceKey: {
    ...
    const response = await this.app.sendEmail({
      ...
      data: {
        message: {
          ...
          track_links: this.trackLinks && parseInt(this.trackLinks),
          track_read: this.trackRead && parseInt(this.trackRead),
          bypass_global: this.bypassGlobal && parseInt(this.bypassGlobal),
          bypass_unavailable: this.bypassUnavailable && parseInt(this.bypassUnavailable),
          bypass_unsubscribed: this.bypassUnsubscribed && parseInt(this.bypassUnsubscribed),
          bypass_complained: this.bypassComplained && parseInt(this.bypassComplained),
