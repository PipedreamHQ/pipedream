...
Super Carl uses API-key authentication. In Super Carl, open **Integrations**, go to the **MCP & API** section, and create an API key with the `search` scope for search actions and the `communications` scope for communication actions. API keys are available on every plan. Paste that key into the Pipedream connected account when prompted.
