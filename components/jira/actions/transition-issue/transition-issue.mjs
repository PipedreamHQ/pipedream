...
export default {
  ...
  description: "Performs an issue transition and, if the transition has a screen, updates the fields from the transition screen. [See docs here](https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-issues/#api-rest-api-3-issue-issueidorkey-transitions-post)",
  version: "0.2.0",
  type: "action",
  props: {
    ...base.props,
  ...
  async run({ $ }) {
    ...
    const fieldsParsed = utils.parseStringToJSON(this.fields);
    ...
    const response = await this.jira.performTranstionIssue({
      $,
    ...
    $.export("$summary", "Successfully performed transition issue");
