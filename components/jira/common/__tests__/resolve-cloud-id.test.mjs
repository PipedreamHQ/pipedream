 ...
 *
 * Run with:
 *   node --experimental-vm-modules node_modules/.bin/jest components/jira/common/__tests__
 *
 * Or directly (no Jest required):
 *   node components/jira/common/__tests__/resolve-cloud-id.test.mjs
 */

import assert from "node:assert/strict";

// Minimal recreation of the _resolveCloudId + _makeRequest logic so the
// tests don't depend on the Pipedream runtime or live credentials.
function buildApp({ getCloudsResult, getCloudsError } = {}) {
  return {
    ...
    async _makeRequest({ url, path, cloudId } = {}) {
      // Mirrors the actual implementation: only resolve cloudId when building
      // the path-based URL. If a full url is passed, skip resolution entirely.
      if (url) {
        return {
