...
export default {
  ...
  description: "Creates a new Asana task by instantiating an existing task template. Use this when a team has a reusable task structure (e.g. a bug-report or onboarding template); for ad-hoc tasks use **Create Task**. Use **List Task Templates** to find the template GID. This kicks off an asynchronous job — the response is a job resource, not the finished task; `new_task.gid` is available immediately but the task's fields may still be populating, so use **Get Task Details By ID** if you need to confirm it's ready. The new task's project and memberships are determined by the template itself, not by any project passed here. Example: call with `taskTemplateId: '1205678901234567'`, `name: 'Bug: Login page crash'` → returns `{gid: '1205555555555555', resource_type: 'job', new_task: {gid: '1206789012345678'}}`. [See the documentation](https://developers.asana.com/reference/instantiatetask)",
  version: "0.0.11",
  ai: "optimized",
  annotations: {
