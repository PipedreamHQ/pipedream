 ...
 *
 * Derived from the Google Health API discovery document, revision 20260824:
 * https://health.googleapis.com/$discovery/rest?version=v4
 *
 * Regenerated by the tooling that produced this app rather than by hand. If you
 * are editing this in the monorepo, change the table below and note that a
 * future regeneration will need the same change applied upstream.
 *
...
export const DATA_TYPES = {
  ...
  "activity-level": {
    ...
    note: "Discovery lists rollup support; the docs table does not. We follow the docs (list/reconcile) as the conservative read",
  },
  "altitude": {
    recordType: "INTERVAL",
  ...
  "nutrition-log": {
    ...
    note: "Docs say record type Sample, but the schema has interval:SessionTimeInterval and no sampleTime — built as SESSION",
  },
  "ovulation-test": {
    recordType: "SAMPLE",
...
/**
 * Rollup-only types, mapped to the tool that does expose them. Keyed by action
 * key rather than display name: this string is handed to an agent in an error
 * message, and the key is what it can actually call.
 */
export const ROLLUP_ONLY_TOOL_HINT = {
  "total-calories": "google_health-get-daily-activity-summary",
  "floors": "google_health-get-daily-activity-summary",
  "calories-in-heart-rate-zone": "google_health-get-daily-activity-summary",
};
