...
module.exports = {
	...
	propDefinitions: {
		...
		template: {
			...
			async options({account}) {
				...
				const { envelopeTemplates } = await this.listTemplates(baseUri, account);
		...
		role: {
			...
			async options({account, template}) {
				const baseUri = await this.getBaseUri(account);
				const { signers } = await this.listTemplateRecipients(baseUri, account, template);
	...
	methods: {
		...
		async getUserInfo() {
			return await this._makeRequest('GET', 'https://account-d.docusign.com/oauth/userinfo');
		},
		...
		async listTemplates(baseUri, accountId) {
			return await this._makeRequest('GET', `${baseUri}/restapi/v2.1/accounts/${accountId}/templates`);
		},
		async listTemplateRecipients(baseUri, accountId, templateId) {
			return await this._makeRequest('GET', `${baseUri}/restapi/v2.1/accounts/${accountId}/templates/${templateId}/recipients`);
		},
		async createEnvelope(baseUri, accountId, data) {
			return await this._makeRequest('POST', `${baseUri}/restapi/v2.1/accounts/${accountId}/envelopes`, data);
		}
	},
}