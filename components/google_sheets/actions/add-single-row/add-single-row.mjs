...
export default {
  ...
  props: {
    googleSheets,
    drive: {
      propDefinition: [googleSheets, "watchedDrive"],
    },
    sheetId: {
      propDefinition: [
        googleSheets,
  ...
  async additionalProps() {
    const { sheetId, worksheetId } = this;

    const props = {};
    if (this.hasHeaders) {
      const worksheet = await this.getWorksheetById(sheetId, worksheetId);

      const { values } = await this.googleSheets.getSpreadsheetValues(
        sheetId,
        `${worksheet?.properties?.title}!1:1`
      );
      if (!values[0]?.length) {
        throw new ConfigurationError(
          'Could not find a header row. Please either add headers and click "Refresh fields" or adjust the action configuration to continue.'
        );
      }
      ...
      props.myColumnData = {
        ...
        label: "Values",
        description:
          "Provide a value for each cell of the row. Google Sheets accepts strings, numbers and boolean values for each cell. To set a cell to an empty value, pass an empty string.",
      };
    }
    return props;
  },
  async run({ $ }) {
    ...
    if (!cells || !cells.length) {
      throw new ConfigurationError(
        "Please enter an array of elements in `Cells / Column Values`."
      );
    }
    cells = parseArray(cells);
    if (!cells) {
      throw new ConfigurationError(
        "Cell / Column data is not an array. Please enter an array of elements in `Cells / Column Values`."
      );
    } else if (Array.isArray(cells[0])) {
      throw new ConfigurationError(
        "Cell / Column data is a multi-dimensional array. A one-dimensional is expected. If you're trying to send multiple rows to Google Sheets, search for the action to add multiple rows to Sheets."
      );
    }

    const { arr, convertedIndexes } =
    ...
    let summary = `Added 1 row to [${sheetName || sheetId} (${
      data.updatedRange
    })](https://docs.google.com/spreadsheets/d/${sheetId}).`;
    if (convertedIndexes.length > 0) {
      summary +=
        " We detected something other than a string/number/boolean in at least one of the fields and automatically converted it to a string.";
    }
