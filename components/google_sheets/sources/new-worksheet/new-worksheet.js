...
module.exports = {
  ...
  description: 'Emits an event each time a new worksheet is created in a spreadsheet.',
  version: '0.0.1',
  dedupe: 'unique',
  props: {
    ...common.props,
    sheetID: {
      propDefinition: [ common.props.google_sheets, 'sheetID', c => ({ driveId: c.watchedDrive === 'myDrive' ? null : c.watchedDrive })]
    },
  },
  hooks: {
    ...common.hooks,
  ...
  methods: {
    ...
    async processSpreadsheet({ spreadsheetId }) {
      const { sheets: worksheets } = await this.google_sheets.getSpreadsheet(spreadsheetId);
