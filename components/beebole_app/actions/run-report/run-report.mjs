  ...
  props: {
    ...
    keys: {
      ...
      options: [
        ...
        { label: 'Billing - %', value: 'billingPercent' },
        { label: 'Billable - ratio', value: 'billingRatio' },
        { label: 'Billable/Non-billable', value: 'billableSplit' },
        { label: 'Billable Utilization %', value: 'billability' },
        { label: 'Budget', value: 'budget' },
        { label: 'Business days', value: 'workDays' },
        { label: 'Business days remaining', value: 'remWorkDays' },
        { label: 'Business hours', value: 'workHours' },                                                
        { label: 'Business hours remaining', value: 'remWorkHours' },
        { label: 'Comment', value: 'comment' },
        { label: 'Company', value: 'company' },
        { label: 'Company ID', value: 'companyId' },      
        { label: 'Costs', value: 'cost' },
        { label: 'Costs - %', value: 'costsPercent' },
        { label: 'Date', value: 'date' },
        { label: 'Daily Cost', value: 'dailyCost' },
        { label: 'Daily Rate', value: 'dailyRate' },      
        { label: 'Days', value: 'inDays' },
        { label: 'Event ID', value: 'eventId' },
        { label: 'Hourly Cost', value: 'hourlyCost' },
        { label: 'Hourly Rate', value: 'hourlyRate' },
        { label: 'Hours', value: 'hours' },
        { label: 'Hours - %', value: 'hoursPercent' },
        { label: 'Is working', value: 'isAtWork' },
        { label: 'Month', value: 'month' },
        { label: 'Overtime / day', value: 'overtime' },
        { label: 'Overtime / week', value: 'weeklyOvertime' },
        { label: 'Person', value: 'person' },
        { label: 'Person ID', value: 'personId' },
        { label: 'Profit', value: 'profit' },
        { label: 'Profit - %', value: 'profitPercent' },
        { label: 'Profit - ratio', value: 'profitRatio' },
        { label: 'Project', value: 'project' },
        { label: 'Project ID', value: 'projectId' },
        { label: 'Status', value: 'status' },
        { label: 'Task', value: 'task' },
        { label: 'Task ID', value: 'taskId' },
        { label: 'Time End', value: 'etime' },
        { label: 'Time Start', value: 'stime' },
        { label: 'Sub Project', value: 'subproject' },
        { label: 'Sub Project ID', value: 'subprojectId' },
        { label: 'Week', value: 'week' },
      ],
      default: ["company", "project", "task", "person", "hours", "comment"],    
    },
    statusFilters: {
      type: "string[]",
      label: "Status",
    ...
    groups: {
      ...
      async options() {
        const response = await this.app.apiRequest({
          data: {
            services: [ { service: "group.tree" }, { service: "custom_field.list"} ]
           }
        })
        let opts = [],
          pushGroups = function(v,c,t) {
            v.forEach((i) => {
              let text = t+i.name;
              if (i.groups) {
                c.push({"value":"gid"+i.id,"label":text});
                pushGroups(i.groups,c,text+": ");
              }
            })
          }
        pushGroups(response[0].groups,opts,"");
        response[1].customFields.forEach((cf) => opts.push({ "value": "gid" + cf.id, "label": cf.name }))
