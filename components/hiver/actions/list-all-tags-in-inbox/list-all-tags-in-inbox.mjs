import hiver from "../../hiver.app.mjs";

export default {
  key: "hiver-list-all-tags-in-inbox",
  name: "List All Tags In Inbox",
  description: "Get all tags associated with a Hiver inbox. [See the documentation](https://developer.hiverhq.com/hiver-api/inbox/get-all-tags-in-the-inbox)",
  version: "0.0.1",
  type: "action",
  annotations: {
    destructiveHint: false,
    openWorldHint: true,
    readOnlyHint: true,
  },
  props: {
    hiver,
    inboxId: {
      propDefinition: [
        hiver,
        "inboxId",
      ],
    },
    limit: {
      type: "integer",
      label: "Page Size",
      description: "Number of tags to fetch per page (10-100). Defaults to 50. All pages are automatically fetched.",
      optional: true,
      min: 10,
      max: 100,
    },
  },
  async run({ $ }) {
    const results = [];
    let nextPage;
    do {
      const { data } = await this.hiver.listInboxTags({
        $,
        inboxId: this.inboxId,
        params: {
          limit: this.limit ?? 50,
          next_page: nextPage ?? undefined,
        },
      });
      results.push(...(data.results ?? []));
      nextPage = data.pagination?.next_page ?? null;
      if (nextPage) await new Promise((r) => setTimeout(r, 1000));
    } while (nextPage);
    $.export("$summary", `Successfully retrieved ${results.length} tag(s) from inbox ${this.inboxId}`);
    return results;
  },
};
