...
export default {
  ...
  propDefinitions: {
    ...
    invoice: {
      type: "object",
      label: "Invoice",
      description: "Structured invoice data (EN 16931 CII XML). **Every nested object is required** — partial seller/buyer/line objects will fail API validation with a generic 400. Required shape:\n\n```json\n{\n  \"number\": \"INV-1001\",\n  \"issueDate\": \"2026-06-16\",\n  \"dueDate\": \"2026-07-16\",\n  \"currencyCode\": \"EUR\",\n  \"seller\": {\n    \"name\": \"Acme GmbH\",\n    \"address\": { \"line1\": \"Hauptstr 1\", \"city\": \"Berlin\", \"postalCode\": \"10115\", \"countryCode\": \"DE\" },\n    \"taxId\": \"DE123456789\"\n  },\n  \"buyer\": {\n    \"name\": \"Example SARL\",\n    \"address\": { \"line1\": \"15 rue de Rivoli\", \"city\": \"Paris\", \"postalCode\": \"75001\", \"countryCode\": \"FR\" }\n  },\n  \"lines\": [{\n    \"description\": \"Consulting\",\n    \"quantity\": 1,\n    \"unitPrice\": 100,\n    \"lineTotal\": 100,\n    \"vatRate\": 20\n  }],\n  \"totalNetAmount\": 100,\n  \"totalTaxAmount\": 20,\n  \"totalGrossAmount\": 120\n}\n```\n\nRequired top-level: `number`, `issueDate`, `currencyCode`, `seller`, `buyer`, `lines`, `totalNetAmount`, `totalTaxAmount`, `totalGrossAmount`. Strongly recommended: `dueDate` (or `paymentTerms`).\n\nRequired on each `seller` / `buyer`: `name` AND `address` (with `line1`, `city`, `postalCode`, `countryCode` all required — ISO 3166-1 alpha-2 country code). For EU B2B, also include `taxId` (VAT ID).\n\nRequired on each item in `lines`: `description` (NOT `name`), `quantity`, `unitPrice`, `lineTotal` (= quantity × unitPrice, net before tax). Optional: `vatRate` (percent, defaults to 0), `vatCategoryCode` (S/Z/E, defaults to S), `unitCode` (UN/ECE code, defaults to C62).",
    },
    eInvoiceVerify: {
      type: "boolean",
      label: "Verify",
