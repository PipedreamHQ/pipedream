...
export default {
  ...
  propDefinitions: {
    ...
    botId: {
      ...
      async options({ teamId }) {
        const { bots } = await this.listBots({ teamId });
        return bots.map((bot) => ({
          value: bot.id,
          label: bot.name,
        }));
  ...
  methods: {
    ...
    async askQuestion({ teamId, botId, question, full_source, format, history, metadata }) {
      return this._makeRequest({
        method: "POST",
    ...
    async createSource({ teamId, botId, type, title, url, file, faqs, scheduleInterval }) {
      return this._makeRequest({
        method: "POST",
