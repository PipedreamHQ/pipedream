...
// TODO Split up the base component and component response
export type V1Component = {
  key: string;
  name: string;
  version: string;
  configurable_props?: any; // only when fetched directly, XXX fix type
};
...
export abstract class BaseClient {
   ...
   * ```
   */
  public async getAccounts(params: GetAccountOpts = {}): Promise<Account[]> {
    const resp = await this.makeConnectRequest<{
      data: Account[];
    }>("/accounts", {
      method: "GET",
      params,
  ...
  public async apps(opts?: GetAppsOpts) {
    ...
    const resp = await this.makeAuthorizedRequest<{ data: AppResponse[]; }>(
      "/apps",
      {
        method: "GET",
        params,
  ...
  public async app(idOrNameSlug: string) {
    ...
    const resp = await this.makeAuthorizedRequest<{ data: AppResponse; }>(url, {
      method: "GET",
    });
    return resp.data;
  }
  ...
  public async components(opts?: GetComponentOpts) {
    ...
    const resp = await this.makeConnectRequest<{ data: V1Component[]; }>(path, {
      method: "GET",
      params,
    });
    return resp.data;
  ...
  public async component({ key }: { key: string; }) {
    const url = `/actions/${key}`; // TODO we don't check type here... but should rename or something (trigger)
    const resp = await this.makeConnectRequest<{ data: V1Component; }>(url, {
      method: "GET",
    });
    return resp.data;
  }
