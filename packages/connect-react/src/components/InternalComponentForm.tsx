...
export function InternalComponentForm() {
  ...
  const { hideOptionalProps, onSubmit } = props
  ...
  return (
    <ErrorBoundary fallback={(err) => <p style={{color: "red"}}>Error: {err.message}</p>}>
      <Suspense fallback={<p>Loading form...</p>}>
        <form {...getProps("componentForm", baseStyles, props)} onSubmit={_onSubmit}>
          ...
          {(!hideOptionalProps && optionalProps.length) ? <div>
            ...
            <div {...getProps("optionalFields", baseOptionalFieldsStyles, props)}>
              {optionalProps.map(([prop, enabled]) => <OptionalFieldButton
                key={prop.name}
                prop={prop}
                enabled={enabled}
