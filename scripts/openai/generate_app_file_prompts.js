...
let methodAppFile = `\n${appFile.replace(/"?propDefinitions"?:(\n|\s)*{[\s\S]*?}\n/gm, 'propDefinitions: {}\n')}\n\n###\n\n`
...
if (propDefPrompt.length < 2048) {
  ...
  fs.appendFileSync('app_file_prop_defs.txt', JSON.stringify({ prompt: propDefPrompt, completion: propDefAppFile }) + '\n');
}
...
if (methodPrompt.length < 2048) {
  ...
  fs.appendFileSync('app_file_methods.txt', JSON.stringify({ prompt: methodPrompt, completion: methodAppFile }) + '\n');
}