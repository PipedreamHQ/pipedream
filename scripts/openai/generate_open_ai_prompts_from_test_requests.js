...
fs.createReadStream('data/app_test_requests.csv')
  ...
  .on('data', (row) => {
    ...
    if (prompt.length + completion.length > 2048) {
      ...
      completion = completion.slice(0, 2048 - prompt.length);
    }
  ...
  .on('end', () => {
    ...
    fs.writeFileSync('test_request_prompts.txt', jsonObjects.map(obj => JSON.stringify(obj)).join('\n'));
  });
