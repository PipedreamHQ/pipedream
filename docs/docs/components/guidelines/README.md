...
You can add additional subheadings to each of the top level `Overview`, `Getting
Started` and `Troubleshooting` headings:
...
but but label is set to “Search Term”.
...
for components. They let you render props in the Pipedream dynamically, based on
the value of other props, and can be used to collect more specific information
that can make it easier to use the component. See the Google Sheets example in
the linked component API docs.
