...
<Callout type="info">
If you've never developed Pipedream components before, [start here](/core/contributing/components/).
</Callout>
...
6. Use [the Pipedream CLI](/core/cli/reference/) to `pd publish` or `pd dev` the JavaScript components emitted by step 5 by the full path to the file.
