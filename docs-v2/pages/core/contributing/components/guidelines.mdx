...
These guidelines are specific to [source](/core/workflows/triggers/) development.
...
Instead, [use `@pipedream/platform` axios](/core/workflows/http#platform-axios/).

#### Return JavaScript Objects

When you `return` data from an action, it's exposed as a [step
export](/core/workflows/#step-exports) for users to reference in future steps
...
Pipedream supports a special category of apps called ["databases"](/core/data-management/databases/),
such as
...
2. Proxied execution of commands against a DB, which guarantees that such
   ...
   [shared static IPs docs](/core/workflows/data-management/databases#send-requests-from-a-shared-static-ip))
