...
![Workflow delay step](../images/delay/delay-step-props.png)

#### `$.flow.delay`

If you need to delay a workflow within Node.js code, or you need detailed control over how delays occur, [see the docs on `$.flow.delay`](/core/workflows/code/nodejs/delay/).
...
You'll see the current state of an execution by [viewing its event data](/core/workflows/triggers/inspect/).

#### Cancelling or resuming execution manually

The [**Delay** actions](#delay-actions) and [`$.flow.delay`](/core/workflows/code/nodejs/delay/) return two URLs each time they run:
...
If you use [`$.flow.delay`](/core/workflows/code/nodejs/delay/), you can send these URLs to your own system to handle cancellation / resumption. You can even email your customers to let them cancel / resume workflows that run on their behalf.
