...
Requests to the `resume_url` have [the same limits as any HTTP request to Pipedream](/core/limits/#http-request-body-size), but you can send larger payloads using our [large payload](/core/workflows/triggers/#sending-large-payloads) or [large file](/core/workflows/triggers/#large-file-support) interfaces.
