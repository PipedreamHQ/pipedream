...
Yes, the same `@pipedream/browsers` package can be used in [actions](/core/contributing/components/actions-quickstart/) as well as [sources](/core/contributing/components/sources-quickstart/).
...
For best results, we recommend increasing the amount of memory available to your workflow to 2 gigabytes. You can adjust the available memory in the [workflow settings](/core/workflows/settings/#memory).
