...
Event sources operate primarily as workflow triggers. When you add a new app-based [trigger](/core/workflows/triggers/) to your workflow, you're creating an event source.
...
Once you've created a source, you can use it to trigger [Pipedream workflows](/core/workflows/) or [consume its events](#consuming-events-from-sources) using Pipedream's APIs.

### Creating a source from the CLI

[Download the CLI](/core/cli/install/) and run:
...
You can also trigger a [Pipedream workflow](/core/workflows/) every time your source emits a new event. This lets you run workflows for every new tweet, every new item in an RSS feed, and more.
...
- In real-time, via the CLI's [`pd events` command](/core/cli/reference/#pd-events)
...
Anyone can create an event source or edit an existing one. The code for all event sources is public, and kept in the [`PipedreamHQ/pipedream` repo](https://github.com/PipedreamHQ/pipedream). [Read this quickstart](/core/contributing/components/quickstart/nodejs/sources/) and see the [event source API docs](/core/contributing/components/api/) to learn more about the source development process.
...
Event sources are subject to the [same limits as Pipedream workflows](/core/limits/), except:
