...
You can use pre-built, no-code actions to store, update, and clear data, or interact with data stores programmatically in [Node.js](/core/workflows/code/nodejs/using-data-stores/) or [Python](/core/workflows/code/python/using-data-stores/).
...
The **Key** must evaluate to a string. You can pass a static string, reference [exports](/core/workflows/#step-exports) from a previous step, or use [any valid expression](/core/workflows/using-props/#entering-expressions).
...
To ensure atomic operations, be sure to limit access to a data store key to a [single workflow with a single worker](/core/workflows/settings/concurrency-and-throttling/) or use a service that supports atomic operations from among our [integrated apps](https://pipedream.com/apps).
...
- Adjust your [workflow memory and timeout settings](/core/workflows/settings/) according to the size of the data in your data store. Set the memory at 512 MB and timeout to 60 seconds and adjust higher if needed.
