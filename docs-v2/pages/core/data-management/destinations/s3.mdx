...
You can send data to an S3 Destination in [Node.js code steps](/core/workflows/code/nodejs/) using `$.send.s3()`.
...
If you're authoring a [component action](/core/contributing/components/#actions), you can deliver data to an S3 destination using `$.send.s3`.
