...
You can send data to an SSE Destination in [Node.js code steps](/core/workflows/code/nodejs/) using the `$.send.sse()` function.
...
If you're authoring a [component action](/core/contributing/components/#actions), you can send events to an SSE destination using `$.send.sse`.
