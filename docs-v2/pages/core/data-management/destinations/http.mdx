...
You can send HTTP requests in [Node.js code steps](/core/workflows/code/nodejs/) using `$.send.http()`.
...
If you're authoring a [component action](/core/contributing/components/#actions), you can deliver data to an HTTP destination using `$.send.http`.
...
If you need to access the HTTP response data in your workflow, [use `axios`](/core/workflows/code/nodejs/http-requests/) or another HTTP client.
...
<Callout type="info">
These IP addresses are tied to **requests sent with `$.send.http` only, not other HTTP requests made from workflows**. To whitelist standard HTTP requests from Pipedream workflows, [use VPCs](/core/vpc/).
</Callout>
