...
You can send data to an Email Destination in [Node.js code steps](/core/workflows/code/nodejs/), too, using the `$.send.email()` function. **This allows you to send emails to yourself programmatically, if you need more control than actions provide**.
...
If you're authoring a [component action](/core/contributing/components/#actions), you can deliver data to an email destination using `$.send.email`.
