...
- Only workflows can run in VPCs (other resources like sources or data stores are not currently supported). For example, [sources](/core/workflows/triggers/) cannot yet run in VPCs.
...
- Workflows running in a VPC will still route specific requests routed through [the shared Pipedream network](/core/data-management/destinations/http/#ip-addresses-for-pipedream-http-requests):
