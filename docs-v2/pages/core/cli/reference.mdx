...
[See the CLI installation docs](/core/cli/install/) to learn how to install the CLI for your OS / architecture.
...
[Read more about authoring your own event sources](/core/contributing/components/quickstart/nodejs/sources/).
...
Creates a directory and [an app file](/core/contributing/components/guidelines/#app-files) from a template
...
Creates a new directory and [a component action](/core/contributing/components/#actions) from a template.
...
Creates a new directory and [an event source](/core/workflows/triggers/) from a template.
...
Log in to Pipedream CLI and persist API key locally. See [Logging into the CLI](/core/cli/login/) for more information.

### `pd logout`

Unsets the local API key tied to your account.

Running `pd logout` without any arguments removes the default API key from your [config file](/core/cli/reference/#cli-config-file).
...
Running `pd logs <source-id-or-name>` connects to the [SSE logs stream tied to your source](/core/workflows/triggers/), displaying new logs as the source produces them.
...
Sign up for Pipedream via the CLI and persist your API key locally. See the docs on [Signing up for Pipedream via the CLI](/core/cli/login/#signing-up-for-pipedream-via-the-cli) for more information.
...
When you [login to the CLI](/core/cli/login/), the CLI writes the API key for that account to your config file, in the `api_key` field:
...
If you're working with resources in an [workspace](/core/workspaces/), you'll need to add an `org_id` to your profile.

1. [Retrieve your workspaces's ID](/core/workspaces/#finding-your-workspaces-id)
