...
- **Publish an action from a Node.js code step**. [Publish any Node.js code step as a reusable action](/workflows/visual-builder/code/nodejs/sharing-code/#publish-an-action-from-a-nodejs-code-step) from the Pipedream dashboard.
...
After publishing a new version, all other steps using this same action will have the option to [update to the latest version](/workflows/visual-builder/actions#updating-actions-to-the-latest-version).
