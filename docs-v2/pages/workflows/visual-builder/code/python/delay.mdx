...
Use `pd.flow.delay` to [delay a step in a workflow](/workflows/visual-builder/control-flow/delay).

These docs show you how to write Python code to handle delays. If you don't need to write code, see [our built-in delay actions](/workflows/visual-builder/control-flow/delay#delay-actions).
...
If your workflow's [execution timeout limit](/workflows/visual-builder/settings/#execution-timeout-limit) is set to longer than [default limit](/workflows/limits/#time-per-execution), it may incur more than two [credits](/pricing/#credits) when using `pd.flow.delay`.
</Callout>
