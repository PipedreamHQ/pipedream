...
You'll see the current state of an execution by [viewing its event data](/workflows/visual-builder/triggers/inspect/).

#### Cancelling or resuming execution manually

The [**Delay** actions](#delay-actions) and [`$.flow.delay`](/workflows/visual-builder/code/nodejs/delay/) return two URLs each time they run:
...
If you use [`$.flow.delay`](/workflows/visual-builder/code/nodejs/delay/), you can send these URLs to your own system to handle cancellation / resumption. You can even email your customers to let them cancel / resume workflows that run on their behalf.
