...
[Actions](/contributing/components/#actions) are reusable steps. When you author an action, you can add it to your workflow like you would other actions, by clicking the **+** button below any step.
...
- **Create an action from code**. Develop your action code on your local filesystem and [publish to your Pipedream account using the Pipedream CLI](/contributing/components/actions-quickstart/).
...
1. Node.js code step actions cannot make use of [app files to further reduce redundancy](/contributing/components/guidelines/#promoting-reusability).
2. Node.js code step actions cannot be published to the [Pipedream Component Registry](/contributing/).
3. Node.js code step actions have a slightly different structure than [action components](/contributing/components/api/#component-api).
