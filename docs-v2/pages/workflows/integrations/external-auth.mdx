...
4. You'll be prompted for all required credentials for the app, often just an `oauth_access_token` or `api_key`. [Find the variable that contains your credentials](/docs/workflows/visual-builder/triggers/#copying-references-to-event-data) and pass them to each field:
...
You can set [the `pd-nostore` flag](/docs/workflows/visual-builder/triggers/#x-pd-nostore) to `1` on requests with credentials to disable logging for those requests only.
</Callout>
...
3. You'll be prompted for all required credentials for the app, often just an `oauth_access_token` or `api_key`. [Find the variable that contains your credentials](/docs/workflows/visual-builder/triggers/#copying-references-to-event-data) and pass them to each field:
