...
You can use pre-built, no-code actions to store, update, and clear data, or interact with data stores programmatically in [Node.js](/workflows/visual-builder/code/nodejs/using-data-stores/) or [Python](/workflows/visual-builder/code/pythonusing-data-stores/).
...
The **Key** must evaluate to a string. You can pass a static string, reference [exports](/workflows/#step-exports) from a previous step, or use [any valid expression](/workflows/visual-builder/using-props#entering-expressions).
...
- Adjust your [workflow memory and timeout settings](/workflows/visual-builder/settings/) according to the size of the data in your data store. Set the memory at 512 MB and timeout to 60 seconds and adjust higher if needed.
