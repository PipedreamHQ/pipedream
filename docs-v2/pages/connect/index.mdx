...
Connect lets your users to authorize API access for Slack, GitHub, and more, directly in your app. You can then retrieve fresh credentials for any account, making authenticated requests on their behalf. Pipedream handles the security of credentials and the whole OAuth flow — no need to manage authorization grants or token refresh yourself.
...
2. A [REST API](#rest-api-overview) to retrieve credentials for your end users — OAuth access tokens, API keys, and other credentials stored by users.
