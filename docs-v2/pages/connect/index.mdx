...
When you add an app that uses OAuth to authenticate, you can choose to use either Pipedream's OAuth client (for testing) or [your own OAuth client](/connected-accounts/oauth-clients). If you'd like to use Pipedream OAuth clients in production, please reach out at `connect@pipedream.com` or in our [Slack community](https://pipedream.com/support).
