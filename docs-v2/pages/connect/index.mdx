...
Pipedream Connect is the easiest way for your users to connect to over {process.env.PUBLIC_APPS}+ APIs, **right in your product**. You can build in-app messaging, CRM syncs, AI-driven products, [and much more](/connect/use-cases), all in a few minutes. Visit [the quickstart](/connect/quickstart) to build your first integration.

Connect enables your users to authorize access for any API, directly in your app. You can then retrieve fresh credentials for any account, making authenticated requests on their behalf. Pipedream handles the security of credentials and the whole OAuth flow — **no need to manage authorization grants or token refresh yourself.**
...
and [much, much more](/connect/use-cases).
...
During the preview phase, **Connect is free to use for any workspace on a paid plan**.
...
- Improve the developer experience and SDK integration
