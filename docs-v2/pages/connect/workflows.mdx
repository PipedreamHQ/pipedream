...
Workflows are sequences of [steps](/workflows/steps) [triggered by an event](/core/workflows/triggers), like an HTTP request, or new rows in a Google sheet.

You can use [pre-built actions](/workflows/actions/) or custom [Node.js](/core/workflows/code/nodejs/), [Python](/core/workflows/code/python), [Golang](/core/workflows/code/go/), or [Bash](/core/workflows/code/bash/) code in workflows and connect to any of our {process.env.PUBLIC_APPS} integrated apps.
...
When you configure [pre-built actions](/workflows/actions) or [custom code that connects to third-party APIs](/core/workflows/code/nodejs/auth), you can link accounts in one of two ways:
...
The most common way to trigger workflows is via HTTP webhook. We strongly recommend [creating a Pipedream OAuth client](/workflows/rest-api/auth#creating-an-oauth-client) and authenticating inbound requests to your workflows.
