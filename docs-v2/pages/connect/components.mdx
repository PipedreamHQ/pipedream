...
Pipedream Connect provides APIs to embed pre-built tools ([triggers and actions](/contributing/components/)) directly in your app,
where you're building a workflow builder or AI-driven app, enabling access to 10,000+ built-in API operations. Enable [your end users](/connect/api/#external-users) (or LLM)
to configure, deploy, and invoke Pipedream triggers and actions for more than {process.env.PUBLIC_APPS} APIs.

## What are triggers and actions?

In Pipedream, we call triggers and actions [components](/contributing/components/), which are self-contained Node.js modules.
Your end users configure the inputs and these components produce a result that's exported as output. These components are developed
and maintained by Pipedream and our community and their source code is available in our [public Github repo](https://github.com/PipedreamHQ/pipedream/tree/master/components).

## Implementation

### Use Pipedream's frontend SDK
- Pipedream provides a frontend React SDK to enable your users to configure and run triggers and actions in your UI
- Style the UI components however you want to match the design of your app, and you can also fork the SDK
- Refer to the `connect-react` [SDK](https://github.com/PipedreamHQ/pipedream/blob/master/packages/connect-react/README.md) to get started

<Callout type="info">
Check out the [public demo app](https://pdrm.co/connect) to see the API and SDK in action. You can also [run it locally and explore the code](https://github.com/PipedreamHQ/pipedream-connect-examples/tree/master/connect-react-demo) for a reference implementation.
</Callout>

### Use the backend SDK or REST API
- Implement your own frontend or ship a LLM-driven configuration with the backend SDK
- See below for the quickstart and examples
...
structure](/contributing/components/api/#component-structure) section in our docs for more
details.
...
(also known as [props](/contributing/components/api/#props) or "properties").
...
unless the values that a prop can take are static or free-form.
...
4. `configured_props`: an object containing the props that have already been
    ...
    (aka `authProvisionId`) that your user has connected to the target app.
...
[dynamic props](/contributing/components/api/#dynamic-props), and they need to be configured
...
supports two types of components: [actions](/contributing/components/api/#actions) and
[sources](/contributing/components/api/#sources) (aka triggers).
