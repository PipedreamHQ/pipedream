...
Pipedream Connect provides APIs to embed pre-built tools ([triggers and actions](/core/contributing/components/)) directly in your application
...
In Pipedream, we call triggers and actions [components](/core/contributing/components/), which are self-contained executable units of code. Your end users configure the inputs and these components produce a
...
Before sending requests to the API, make sure to [authenticate using a Pipedream OAuth client](/core/rest-api/auth/#oauth):
...
structure](/core/contributing/components/api/#component-structure) section in our docs for more
details.
...
(also known as [props](/core/contributing/components/api/#props) or "properties").
...
[props](/core/workflows/using-props/)) to the component.
...
[dynamic props](/core/contributing/components/api/#dynamic-props), and they need to be configured
...
supports two types of components: [actions](/core/contributing/components/api/#actions) and
[sources](/core/contributing/components/api/#sources) (aka triggers).
...
1. `exports`: all the named exports produced by the action, like when calling
   [`$.export` in a Node.js](/core/workflows/code/nodejs/#using-export) component.
