...
<Callout type="info">
If you've never developed Pipedream components before, [start here](/contributing/components/).
</Callout>
...
- `this` is strictly-typed within `methods`, `run`, `hooks`, and everywhere you have access to `this` in [the component API](/contributing/components/api/). But this typing can be improved. For example, we don't yet map props to their appropriate TypeScript type (everything is typed with `any`).
