...
This document is intended for developers who want to author and edit [Pipedream Actions](/contributing/components/#actions). After completing this quickstart, you'll understand how to:
...
<Callout type="info">
If you previously developed actions using Pipedream's UI, we recommend reviewing our [migration guide](/contributing/components/migrating/) after completing this quickstart.
</Callout>
...
The following code represents a simple component that can be published as an action ([learn more](/contributing/components/api/) about the component structure). When used in a workflow, it will export `hello world!` as the return value for the step.
...
Next, let's update the component to capture some user input. First, add a `string` [prop](/contributing/components/api/#props) called `name` to the component.
...
Then add an [app prop](/contributing/components/api/#app-props) to use Pipedream managed auth with this component. For this example, we'll add an app prop for Github:
...
In order to help users understand what's happening with each action step, we recommend surfacing a brief summary with `$summary` ([read more](/contributing/components/api/#actions) about exporting data using `$.export`).
...
You're ready to start authoring and publishing actions on Pipedream! You can also check out the [detailed component reference](/contributing/components/api/#component-api) at any time!
