...
Pipedream charges one credit per {process.env.base_credits_price.seconds }} seconds of compute time at {{ $site.themeConfig.base_credits_price.memory } megabytes of memory (the default) per workflow execution. Credits are also charged for [dedicated workers](/workflows/settings/#eliminate-cold-starts).
...
Adding additional memory capacity to workflows will increase credit usage in intervals of 256 megabytes. For example, doubling the memory of a workflow from {process.env.base_credits_price.memory }} to {{ $site.themeConfig.base_credits_price.memory * 2 } will double the cost of credits in the same execution time.
...
5 credits are incurred, because the workflow ran for a total of 5 executions and under the {process.env.base_credits_price.seconds } seconds threshold (5 executions at 1 credit each).
</Callout>

<Callout type="info" emoji="ℹ️">
A workflow that executes 2 times with {process.env.base_credits_price.seconds + 5 } seconds per run

4 credits are incurred, because each workflow execution exceeded 1 credit time limit of {process.env.base_credits_price.seconds } by 5 seconds.
</Callout>
...
<Callout type="info" emoji="ℹ️">
This free credit per execution **only** applies to sources from the [Pipedream public registry](/sources). If you deploy a private custom source to your account, then all computation time including the inital {process.env.base_credits_price.seconds } seconds for that private source counts toward credits.
</Callout>

<Callout type="info" emoji="ℹ️">
A polling source finishing under {process.env.base_credits_price.seconds } seconds per execution
...
<Callout type="info" emoji="ℹ️">
A polling source finishing over {process.env.base_credits_price.seconds } seconds per execution
...
<Callout type="info" emoji="ℹ️">
A custom source that finished under {process.env.base_credits_price.seconds } seconds per execution
