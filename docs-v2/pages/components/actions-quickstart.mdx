...
Please note the value of the `key` attribute *must be unique* across all components in your account (sources and actions). If it's not unique, the pre-existing component with the matching `key` will be updated.

To get started, save the code to a local `.js` file (e.g., `action.js`) and run the following CLI command:

```bash
pd publish action.js --org_id o_abc123
```

The CLI will publish the component as an action in your account with the key `action_demo`. 
