...
The Pipedream API supports two methods of authentication: [OAuth](/core/rest-api/auth/#oauth) and [User API keys](/core/rest-api/auth/#user-api-keys). **Pipedream recommends using OAuth for most use cases**.
...
Learn more in the [Authentication docs](/core/rest-api/auth/).

### Authenticating as a workspace vs. a user

Pipedream recommends using [OAuth](/core/rest-api/auth/#oauth) to auth against the Pipedream API. OAuth tokens are associated with a workspace, and the API will automatically use the workspace associated with the token.
...
<Callout>
The `org_id` parameter is only required when using [User API keys](/core/rest-api/auth/#user-api-keys). When authenticating with OAuth tokens, the API will automatically use the workspace associated with the token.
</Callout>

When using [User API keys](/core/rest-api/auth/#user-api-keys), some endpoints require you to specify [your workspace ID](/core/workspaces/#finding-your-workspaces-id) you want the operation to take effect in:
...
[Find your workspace / org ID here](/core/workspaces/#finding-your-workspaces-id).
...
[Connected accounts](/core/integrations/connected-accounts/) let you manage credentials for integrated APIs.
...
1. [Connect your account](/core/integrations/connected-accounts/#connecting-a-new-account)
...
<Callout>
`/components` endpoints are only available when using [user API keys](/core/rest-api/auth/#user-api-keys), not yet for workspace [OAuth tokens](/core/rest-api/auth/#oauth).
</Callout>
...
The full code for a [Pipedream component](/core/contributing/components/api/).
...
endpoint](/core/rest-api/#get-source-events), and are tied to the timestamp at which
...
as workflow triggers. [Read more here](/core/workflows/triggers/).

### Create a Source

<Callout>
This endpoint is only available when using [user API keys](/core/rest-api/auth/#user-api-keys), not yet for workspace [OAuth tokens](/core/rest-api/auth/#oauth).
</Callout>
...
endpoints](/core/rest-api/#components) for information on how to retrieve this ID.
...
slug](/core/contributing/components/api/#component-structure)
...
component](/core/contributing/components/api/).
...
If absent, this defaults to using the [name slug](/core/contributing/components/api/#component-structure)
of the component used to create the source.
...
<Callout type="warning">
  The Subscriptions API is currently incompatible with projects that have [GitHub Sync](/core/git/) enabled. To [trigger another workflow](/core/workflows/code/nodejs/#invoke-another-workflow), use `$.flow.trigger` instead.
</Callout>
...
[See the component endpoints](/core/rest-api/#components) for information on how to
...
See [the `this.$emit` docs](/core/contributing/components/api/#emit) for more information on how to emit events on custom channels.
...
<Callout>
These endpoints only work when using [user API keys](/core/rest-api/auth/#user-api-keys), and will not work with workspace-level OAuth clients.
</Callout>
...
For example, you can run a Twitter [event source](/core/workflows/triggers/) that listens
...
[**See these tutorials**](/core/rest-api/webhooks) for examples.
...
You can invoke workflows by making an HTTP request to a workflow's HTTP trigger. [See the docs on authorizing requests and invoking workflows](/core/workflows/triggers/#authorizing-http-requests) for more detail.
...
[Switch to your workspace's context](/core/workspaces/#switching-between-workspaces) and [find your org's ID](/core/workspaces/#finding-your-workspaces-id).
...
`org_id` **string**
[Find your org's ID](/core/workspaces/#finding-your-workspaces-id).
...
[`$send.emit()`](/core/data-management/destinations/emit/#emit-events).
...
[Workspaces](/core/workspaces/) provide your team a way to manage resources in a shared workspace. Any resources created by the workspace are owned by the workspace and accessible to its members.
